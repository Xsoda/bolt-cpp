...
std::tuple<std::optional<bolt::pair>, std::uint32_t> Cursor::keyValue() {
    auto ref = stack.back();
    if (ref.count() == 0 || ref.index >= ref.count()) {
        return std::make_tuple<std::optional<bolt::pair>, std::uint32_t>(std::nullopt, 0);
    }

    if (ref.node != nullptr) {
        auto inode = ref.node->inodes.at(ref.index);
        auto kv = std::make_pair(inode.key, inode.value);
        return std::make_tuple(std::make_optional(kv), inode.flags);
    }

    bolt::leafPageElement *elem = ref.page->leafPageElement(ref.index);
    auto kv = std::make_pair(elem->key(), elem->value());
    return std::make_tuple(std::make_optional(kv), elem->flags);
}
