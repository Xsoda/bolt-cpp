...
struct TxStats {
    ...
    TxStats()
        : PageCount(0), PageAlloc(0), CursorCount(0), NodeCount(0), NodeDeref(0), Rebalance(0),
          RebalanceTime(0ms), Split(0), Spill(0), SpillTime(0ms), Write(0), WriteTime(0ms){};
...
struct Stats {
    ...
    Stats() : FreePageN(0), PendingPageN(0), FreeAlloc(0), FreelistInuse(0), TxN(0), OpenTxN(0){};
    ~Stats() = default;
...
struct BucketStats {
    ...
    BucketStats()
        : BranchPageN(0), BranchOverflowN(0), LeafPageN(0), LeafOverflowN(0), KeyN(0), Depth(0),
          BranchAlloc(0), BranchInuse(0), LeafAlloc(0), LeafInuse(0), BucketN(0), InlineBucketN(0),
          InlineBucketInuse(0){};
    ~BucketStats() = default;
...
template <> struct formatter<bolt::TxStats> : nested_formatter<fmt::string_view> {
    auto format(bolt::TxStats &stats, format_context &ctx) const -> decltype(ctx.out()) {
        return write_padded(ctx, [this, &stats](auto out) -> decltype(out) {
            ...
            out = fmt::format_to(out, "Split: {}, Spill: {}, SpillTime: {}\n", stats.Split,
                                 stats.Spill, stats.SpillTime);
            out = fmt::format_to(out, "Write: {}, WriteTime: {}", stats.Write, stats.WriteTime);
            return out;
        });
    };
};

template <> struct formatter<bolt::Stats> : nested_formatter<fmt::string_view> {
    auto format(bolt::Stats &stats, format_context &ctx) const -> decltype(ctx.out()) {
        return write_padded(ctx, [this, &stats](auto out) -> decltype(out) {
            out = fmt::format_to(out, "FreePageN: {}, PendingPageN: {}\n", stats.FreePageN,
                                 stats.PendingPageN);
            out = fmt::format_to(out, "FreeAlloc: {}, FreelistInuse: {}\n", stats.FreeAlloc,
                                 stats.FreelistInuse);
            out = fmt::format_to(out, "TxN: {}, OpenTxN: {}\n", stats.TxN, stats.OpenTxN);
...
template <> struct formatter<bolt::BucketStats> : nested_formatter<fmt::string_view> {
    auto format(bolt::BucketStats &stats, format_context &ctx) const -> decltype(ctx.out()) {
        return write_padded(ctx, [this, &stats](auto out) -> decltype(out) {
            ...
            out = fmt::format_to(out, "InlineBucketN: {}, InlineBucketInuse: {}",
