...
TestResult TestPageType() {
    bolt::page page;
    page.flags = bolt::branchPageFlag;
    if (page.type() != "branch") {
...
TestResult TestMergePgid() {
    auto a = std::vector<bolt::pgid>({4, 5, 6, 10, 11, 12, 13, 27});
    auto b = std::vector<bolt::pgid>({1, 3, 8, 9, 25, 30});
    std::vector<bolt::pgid> c;
    c.assign(a.size() + b.size(), -1);
    bolt::mergepgids(c, a, b);
    auto ret = std::vector<bolt::pgid>(
    ...
    a = std::vector<bolt::pgid>({4, 5, 6, 10, 11, 12, 13, 27, 35, 36});
    b = std::vector<bolt::pgid>({8, 9, 25, 30});
    c.assign(a.size() + b.size(), -1);
    bolt::mergepgids(c, a, b);
    ret = std::vector<bolt::pgid>(
